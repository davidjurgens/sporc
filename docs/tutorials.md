...
and ship **executed**, so you can read them (with their figures) on GitHub
...
!!! tip "If you only read one"
    ...
    since there are no word timings in SPoRC, and shows how to re-derive them from
    source audio.
...
1. **SPoRC is two months.** Every episode is from 1 May – 30 June 2020. There are
   no long-run trends, but a sharp dateable event sits mid-window, which notebook
   05 uses.
2. **~65% of episodes have speaker turns.** `len(episode.turns) == 0` is
   ambiguous, so gate on `episode.has_turn_data` instead.
